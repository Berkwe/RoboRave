...
// * PID değişkenleri
float g_Kp = 120; // ? pcb sayesinde ne verirsen gidiyo sıfır göcünme
float g_Ki = 0; // ? arttırınc bi faydasını göremedim robot genelde sağa kayıyor fakat düzletmekte çok osilasyona sebep oldu
float g_Kd = 21; // ? 145 kp ve 130 hızında ideal olanlar 10 ve 10'un katları. 21 verdim farketmiyor sanırsam 10'un katı olduğu sürece
int g_baseSpeed = 130; // ? 200 hızında bile çalışıyor pcby sayesinde
...
void calibrateSensors() { // ? Ir sensörleri arka planla kalibre eder
    ...
    // ! ###
    if (localLineDetectionValueAvarge>backgroundDetectionValueAvarge) isReversedLines = true; // ? eğer çizgi açık bir renk ise
